...
{
    public class CratePig : BaseStatedBody<CratePig.State>
    {
        public bool Hit;
        public Vector2 Dir;
        ...
        public void ThrowCrate()
        {
            ...
            var crateI = Scenes.FlyingCrate.Instance<FlyingCrate>();
            crateI.GlobalPosition = GlobalPosition;
            parent.CallDeferred("add_child", crateI);
            var factor = Dir.y < .1f ? .3f : .8f;
            crateI.CallDeferred("apply_central_impulse", Dir * crateI.Weight * 1.5f + Vector2.Up * crateI.Weight * factor);
        }
        ...
        public override void OnHitBoxEnter(Area2D area)
        {
            ...
            if (isDmgFromAttack && InvincibleTimer <= 0 && !Hit)
            {
                Hit = true;
                var pigI = Scenes.Pig.Instance<Pig>();
                var crateI = Scenes.Crate.Instance<Crate>();
                var parent = GetParent();
                pigI.GlobalPosition = GlobalPosition;
                crateI.GlobalPosition = GlobalPosition;
                pigI.Dmged = true;
